...
def test_real_egress_proxy_enforces_network_and_allowlist() -> None:
    ...
    assert "Location:" not in report.redirect_denied.stdout
...
def test_real_envoy_egress_proxy_profile_enforces_network_and_allowlist() -> None:
    require_docker()
    ...
    report = DockerEgressProxyVerifier(
        ...
        allowed_ports=[8080],
        proxy_kind="envoy",
        proxy_image_ref="envoyproxy/envoy:v1.35-latest",
    ).run()
    ...
    assert "envoy_cluster_upstream" in report.metrics.stdout
    assert report.admin_from_client.exit_code != 0
    assert report.proxy_unavailable.exit_code != 0
