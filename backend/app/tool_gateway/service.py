...
class ToolGatewayServiceError(RuntimeError):
    ...
    async def invoke(
        ...
        *,
        ...
        if _requires_approval(authorized_tool):
            ...
            await _record_audit_event(self.audit_store, invocation=invocation, result="pending")
            ...
            return _build_invocation_response(invocation, approval_task=approval_task)
        ...
        if server.credential_ref:
            if server.credential_ref_id is None:
                await _record_inactive_credential_failure(
                    self.audit_store,
                    self.invocation_store,
                    project_id=project_id,
                    actor_id=actor_id,
...
async def _record_inactive_credential_failure(
    ...
    *,
    ...
    server: ToolMcpServerCredentialRead,
    started: float,
) -> None:
    invocation = await _record_invocation(
        invocation_store,
